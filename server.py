...
def handle_inputs():
    ...
    try:
        ...
        if msg.startswith("input:"):
            comando = msg.split(":", 1)[1]   # <- limit split to 1
            
            # Se ambos clientes conectados e alguém pressiona SPACE
            if comando == "SPACE" and len(clients) == 2:  # <- usar 2 aqui
                started = True
                print("Jogo iniciado!")
            else:
                inputs[addr] = comando
...
def send_state():
    ...
    state = {
        ...
        "score1": score1,
        "score2": score2,
        "started": started,   # <- incluir started no estado enviado
    }
